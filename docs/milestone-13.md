...
- Wired flows: `init`, `transfer`, `change-policy`, `rotate-ed25519`,
  `recover {enable,disable,rotate-ed25519}`.
...
- Legacy transaction assembly for HybridAnd TransferSol (unit-tested without
  RPC). Oversized Falcon rotate paths may still need ALT/v0 manually.
...
Live localnet smoke is optional (`#[ignore]` / documented in README under M16).
